 ...
 * ------------------------------------------------------------------------------------------ */
import { SignatureHelpProvider, SignatureHelp, SignatureInformation, CancellationToken, TextDocument, Position, workspace, MarkdownString } from 'vscode';
import pxpplusGlobals = require('./pxplusGlobals');
import pxpplusGlobalFunctions = require('./pxplusGlobalFunctions');
...
export default class PxPlusSignatureHelpProvider implements SignatureHelpProvider {
	...
	public provideSignatureHelp(document: TextDocument, position: Position, _token: CancellationToken): Promise<SignatureHelp> | null {
        ...
        entry.signatures.forEach((value:pxpplusGlobals.ISignature) => {
            let signatureInfo = new SignatureInformation(ident + value.signature, textToMarkdownString(value.description || entry.description || ''));
            value.parameters.forEach(element => {
                signatureInfo.parameters.push({
                    label: element.parameter ?? '',
                    documentation: textToMarkdownString(element.description || '')
                })
            });
